...
    super(props)
    ...
    this.state = {
      ...
      originalData: data,
      filter: "",
      sort: {sortable: this.props.defaultSort, direction: undefined},
      summaries: undefined,
      savedFilters: []
    }
...
  }
  ...
  componentWillReceiveProps(props){
    ...
    data = this.sortArray(data, this.state.sort.sortable, this.state.sort.direction)
      ...
      if (this.props.sortables && this.props.sortables[data.id]){
        ...
        sortButtons =
          <div className="order-actions">
            <span key="arrow-asc" className="order-arrow asc" onClick={() => this.onClickSort(data.id, "asc")}></span>
            <span key="arrow-desc" className="order-arrow desc" onClick={() => this.onClickSort(data.id, "desc")}></span>
          </div>
      }
  ...
  onClickSort(id, direction){
    let sortable = this.props.sortables[id]
    this.setState({data: this.sortArray(this.state.data, sortable, direction), sort: {sortable: sortable, direction: direction}})
  }
  ...
  filterArray(strings, array, filters){
    let arr = [...array]
    let data = arr.filter((data) => this.filterMultiple(data, strings, filters))
...
  render() {
    ...
    let rows = this.state.data.length > 0 ? this.state.data.map((row, key) => {
      let cells = this.props.dataKeys.map((cell, key) =>
        <td key={key} className={this.setClass(["data-table-desktop-cell", cell.id])} onClick={cell.onClick ? () => cell.onClick({cell: cell, row: row}) : undefined}>
          {cell.value(row)}
        </td>
      )
      ...
      if (this.props.actions && this.props.actions.length > 0){
        let actions = this.props.actions.map((data, key) =>
          <div key={key} className={["action", data.className ? data.className(row) : null].join(" ")} onClick={() => data.action(row)}>
      ...
      return(
        <tr key={key} className={this.setClass(rowClass)} onClick={() => this.props.onClickRow ? this.props.onClickRow(row) : undefined}>{cells}</tr>
      )
    }) :
